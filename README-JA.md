...
> **コマンド名について:** コマンドパレットの項目・通知・ステータスバーは、設定で選んだUI言語（English、한국어、日本語）に従います。Obsidianは読み込み時にパレットをキャッシュするため、言語を切り替えた後は新しい名前を反映させるためにアプリを再起動してください。
...
- **多言語UI** — English、한국어、日本語。設定・サイドバー・コマンドパレット・通知・ステータスバーがすべて選択した言語に従います
...
リボンアイコンをクリックするか、コマンドパレットから**アシスタントを開く**(「アシスタントを開く」)を実行
