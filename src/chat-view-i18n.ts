...
export const VIEW_I18N = {
  en: {
    ...
    // TimeBox 관련 키 (Create_Timebox_Action)
    createTimebox: "Create TimeBox",
    timeboxNoTodo: "Create today's To-Do first before creating a TimeBox.",
    timeboxExists: (path: string) => `TimeBox already exists: ${path}`,
    timeboxCreated: (path: string) => `TimeBox created: ${path}`,
    timeboxFallback: "AI drafting failed. Created a TimeBox from the template instead.",
    timeboxError: (e: string) => `TimeBox creation failed: ${e}`,
    timeboxInProgress: "A TimeBox for this date is already being created.",
...
${content}`,
  ...
  ko: {
    ...
    // TimeBox 관련 키 (Create_Timebox_Action)
    createTimebox: "TimeBox 생성",
    timeboxNoTodo: "TimeBox를 만들기 전에 먼저 오늘 To-Do를 생성하세요.",
    timeboxExists: (path: string) => `TimeBox가 이미 존재합니다: ${path}`,
    timeboxCreated: (path: string) => `TimeBox 생성됨: ${path}`,
    timeboxFallback: "AI 초안 작성에 실패하여 템플릿 기반으로 TimeBox를 생성했습니다.",
    timeboxError: (e: string) => `TimeBox 생성 실패: ${e}`,
    timeboxInProgress: "이 날짜의 TimeBox를 이미 생성하고 있습니다.",
...
${content}`,
  ...
  ja: {
    ...
    // TimeBox 関連キー (Create_Timebox_Action)
    createTimebox: "TimeBox作成",
    timeboxNoTodo: "TimeBoxを作成する前に、先に今日のTo-Doを作成してください。",
    timeboxExists: (path: string) => `TimeBoxは既に存在します: ${path}`,
    timeboxCreated: (path: string) => `TimeBox作成完了: ${path}`,
    timeboxFallback: "AIの下書き作成に失敗したため、テンプレートからTimeBoxを作成しました。",
    timeboxError: (e: string) => `TimeBox作成失敗: ${e}`,
    timeboxInProgress: "この日付のTimeBoxは既に作成中です。",
