 ...
 *
 * **지울 것만** 열거한다(허용 목록이 아니다). 두 방식의 실패 방향이 다르다:
 *  - 허용 목록에서 빠뜨린 문자 → 지워짐 → 서로 다른 결정이 한 키가 됨 → **항목 손실**
 *  - 지울 목록에서 빠뜨린 문자 → 남음 → 같은 결정이 두 항목이 됨 → 중복 행
 *
 * 손실이 중복보다 나쁘므로 안전한 쪽으로 실패하는 지울 목록을 쓴다. 실제로 허용 목록으로
 * 뒀을 때 `#`·`+`·`<`·`>`를 넣고도 `=`를 빠뜨려 `A > B`와 `A >= B`가 한 키가 됐다.
 *
 * 지우는 것은 문장 장식뿐이다 — 마침표·쉼표·느낌표·물음표·따옴표·괄호·중점.
 * 연산자·언어 이름에 쓰이는 기호(`# + - * / < > = & ~ ^ %`)는 전부 남는다.
 */
const DECISION_KEY_DECORATION = /[.,!?;:'"“”‘’«»()[\]{}·…]+/gu;

export function decisionKey(decision: string): string {
  return decision
    .toLowerCase()
    .replace(DECISION_KEY_DECORATION, " ")
    .replace(/\s+/g, " ")
    .trim();
}
