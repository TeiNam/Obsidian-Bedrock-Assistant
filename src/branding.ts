...
//
// 이 에디션은 AWS Bedrock 단일 백엔드만 지원하므로
// 브랜딩(displayName, icon, settingsTitle)도 Bedrock(Assistant Kiro) 고정입니다.
...
// ============================================
// 브랜딩을 반환하는 함수 (Bedrock 단일)
// ============================================

/**
 * Bedrock 단일 백엔드 브랜딩(displayName, icon, settingsTitle)을 반환한다.
 * pluginId, viewType, files는 포함하지 않음 (고정값이므로).
 */
export function getBranding(): SwitchableBranding {
  return BEDROCK_BRANDING;
}

// ============================================
// BRANDING: 기본 내보내기 (Bedrock 고정)
// ============================================
...
/**
 * BRANDING 객체의 displayName, icon, settingsTitle을 Bedrock 브랜딩으로 갱신한다.
 * pluginId, viewType, files는 변경하지 않음 (고정값).
 * Object.assign으로 내부 값만 갱신하므로 기존 import 참조가 유지된다.
 * (단일 백엔드이므로 항상 동일한 값으로 갱신되며, 호출 호환을 위해 함수는 유지한다.)
 */
export function updateBranding(): void {
