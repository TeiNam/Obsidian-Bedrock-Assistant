...
// 백엔드(Bedrock, Gemini, OpenAI, Ollama)에 따라 displayName과 icon을
// 런타임에 동적으로 전환합니다.
...
/** 백엔드별 전환 가능한 브랜딩 필드 (displayName, icon) */
type SwitchableBranding = Pick<BrandingConfig, "displayName" | "icon">;
...
/**
 * 지정된 AI 백엔드에 해당하는 브랜딩(displayName, icon)을 반환한다.
...
/**
 * BRANDING 객체의 displayName과 icon을 지정된 백엔드에 맞게 갱신한다.
